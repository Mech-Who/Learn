...
bool is_elem(vector<elemType> &vec, elemType elem){
    // int max_value = max_element(vec.begin(), vec.end());
    // 由于vec本身是个有序递增的数列，因此，并不需要真的用到max_element
    int max_value = vec.empty()? 0: vec[vec.size()-1];
    if (max_value < elem)
        return grow_vec(vec, elem);
    if (max_value == elem)
        return true;
    // binary_search要求序列是有序的，如果不确定是否有序，则可以先复制一份进行排序
    vector<elemType> temp(vec.size());
    copy(vec.begin(), vec.end(), temp.begin());
    sort(temp.begin(), temp.end())
    return binary_search(temp.begin(), temp.end(), elem);
}

bool grow_vec(std::vector<int>&, int){
